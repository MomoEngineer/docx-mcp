...
| [0008](0008-scoped-paragraph-range-reads.md) | Scoped Paragraph-Range Reads for `read_document` and `get_structure` | Accepted |
...
- [ADR-0006 §Decision.5](0006-phase-5-paragraph-edit-module-layout.md#decision) deferred cleaning up `delete_paragraph`'s now-orphaned `<w:footnote>` entry in `word/footnotes.xml`, "once real footnote-editing infrastructure exists," and its own Consequences named this an open question for Phase 6's ADR to pick up. [ADR-0007](0007-phase-6-footnote-edit-module-layout-and-multi-part-atomic-write.md) (Phase 6) shipped `add_footnote`/`edit_footnote` without revisiting it — `delete_paragraph` itself was untouched in Phase 6, and no ADR since has taken up the extension. This was not a considered rejection (there is no Alternatives entry for it in any ADR), so it is recorded here, explicitly, as a still-open-but-unscheduled possible future enhancement rather than left for a reader to infer from ADR-0006's now-stale "Phase 6" wording: a future change that does take it on gets its own ADR: none is currently planned.
- [ADR-0008](0008-scoped-paragraph-range-reads.md)'s Consequences records a **deliberately deferred** idea from the same review that produced it: letting `insert_paragraph` attach one or more footnotes in the same call, to cut the round-trip count when drafting a citation-heavy paragraph. No current evidence shows the write-side round-trip count is actually a bottleneck; a future change that takes it on gets its own ADR once real usage shows it matters.
